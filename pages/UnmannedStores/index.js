...
Page({
  ...
  initList() {
    const param = {
      [u_houseFlowList.page]: this.data.g_page,
    };
    byGet(getApp().data.k1swUrl + u_houseFlowList.URL, param).then(response => {
  ...
  handleSubmit() {
    const apiUrls = {
      getCarStatus: getApp().data.k1swUrl + u_houseFlowApirAdd.URL
    };
    const param = {
      ...this.data.params,
  ...
  handleDelete(evt) {
    ...
    byPost(getApp().data.k1swUrl + u_unmannedStoreApidel.URL, info, (response) => {
      if (response?.data?.code == 1000) {
