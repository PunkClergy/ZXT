...
Page({
  ...
  initList() {
    const param = {
      [u_safekeyapiList.page]: this.data.g_page,
    };
    byGet(getApp().data.k1swUrl + u_safekeyapiList.URL, param).then(response => {
  ...
  handleSubmit() {
    const apiUrls = {
      getCarStatus: getApp().data.k1swUrl + u_safekeyapiaddOrUpdate.URL
    };
    const param = {
      ...this.data.params,
      type:this.data.batterylift
    };
    showLoading();
  ...
  handleEdit(evt) {
    ...
    this.setData({
      ...
      batterylift: info?.batterylift || 1,
    })
  },
  // 切换tabs标签
  handleSwitchTab(e) {
