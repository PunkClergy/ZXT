...
Page({
  ...
  handleFormSubmit(evt) {
    ...
    const requestParams = {
      ...
      multipleUsed: this?.data?.pickerList?.[this?.data?.pickerIndex]?.value
    };
  ...
  handleEditKey(evt) {
    console.log(evt)
    this.setData({
      radioValue: evt?.currentTarget?.dataset?.item?.client || 0,
      c_edit_key_show_momal: true,
      g_edit_info: evt.currentTarget.dataset.item,
      pickerIndex: Number(evt.currentTarget.dataset.item?.multipleUsed) == 0 ? 1 : 0
    })
