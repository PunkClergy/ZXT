...
Page({
  ...
  onLoad(options) {
    if (options.item) {
      ...
      this.setData({
        page_nature: options?.flag,
        params,
        brakingType: info?.brakingType, //自助取还 1 ：自助取还 2 非自助取还
        batterylift: info?.batterylift, //启动方式
        carOwnerName: info?.carOwnerName || '', //所属平台
        carOwnerNameValue: info?.carOwnerName == '智信通' ? '' : info?.carOwnerName
      })
    }
