...
Page({
  ...
  initList() {
    const param = {
      [u_carStoreList.page]: this.data.g_page,
    };
    byGet(getApp().data.k1swUrl + u_carStoreList.URL, param).then(response => {
  ...
  handleSubmit() {
    const apiUrls = {
      getCarStatus: getApp().data.k1swUrl + u_carStoreApiaddOrUpdate.URL
    };
    const param = {
      ...this.data.params,
  ...
  handleDelete(evt) {
    ...
    byPost(getApp().data.k1swUrl + u_carStoreApidel.URL, info, (response) => {
      if (response?.data?.code == 1000) {
  ...
  handleSeach() {
    ...
    byGet(getApp().data.k1swUrl + u_carList.URL, {}).then(response => {
      if (response.statusCode == 200) {
        // 先组合文本，生成带 label 的新数组
        const pickerOptions = response?.data?.content?.map(item => {
          const serial = item.vehicleSerialName || '';
          const mode = item.vehicleModeName || '';
          const platenumber = item?.platenumber || ''
          return {
            ...item,
            label: `${platenumber} - ${serial} - ${mode}`  // 组合后的显示文本
          };
        });
        this.setData({ pickerOptions });
      }
    })
    // 查询门店
    byGet(getApp().data.k1swUrl + u_houseFlowList.URL, {}).then(response => {
      if (response.statusCode == 200) {
        // 先组合文本，生成带 label 的新数组
        const md_pickerOptions = response?.data?.content?.map(item => {
          const serial = item.name || '';
          return {
            ...item,
            label: `${serial}`  // 组合后的显示文本
          };
        });
        this.setData({ md_pickerOptions });
      }
    })
  },
  handleClOnPickerChange(evt) {
    const index = evt?.detail?.value
    const info = this?.data?.pickerOptions?.[index]
    const params = this.data.params
  ...
  handleMdOnPickerChange(evt) {
    const index = evt?.detail?.value
    const info = this?.data?.md_pickerOptions?.[index]
    console.log(info)
    const params = this.data.params
    const { id, name } = info
    const returnValue = Object.assign(params, { storeid: id, storeName: name, })
    this.setData({
      params: returnValue
    })
  },
